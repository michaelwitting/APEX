...
ids <- read.csv(ids_fl, sep = ";")[1:7]

colnames(ids) <- c("Row","Annotation.ID","formula" ,"adduct" , "evidence",  "m.z", "RT"  )
...
# get all manual MOGL species that will be used as seed node set (SNS)
ascr <- ids[grepl("#", ids$Annotation.ID) & 
              !grepl("ascr", ids$Annotation.ID) &
...
write_csv(results_top, 
          file = paste0(path, "results_MOGLs/MOGLs_results_top.csv"))
